...
export class ResponseNormalizer {
  ...
  normalizeToArray<T>(response: unknown, options?: NormalizationOptions): T[] {
    ...
    try {
      ...
      if (response === null || response === undefined) {
        if (opts.logWarnings) {
          this.logger?.debug(
            'Received null/undefined response, returning empty array'
          );
        }
        return [];
      }
      ...
      if (this.isDirectArray(response)) {
        this.logger?.debug('Normalized direct array response', {
      ...
      if (this.isApiResponse(response)) {
        ...
        if (Array.isArray(data)) {
          this.logger?.debug('Normalized API wrapper with array data', {
      ...
      if (response && typeof response === 'object') {
        ...
        if (responseObj.items && Array.isArray(responseObj.items)) {
          this.logger?.debug('Normalized items property', {
            itemCount: responseObj.items.length,
          });
        ...
        if (responseObj.results && Array.isArray(responseObj.results)) {
          this.logger?.debug('Normalized results property', {
            itemCount: responseObj.results.length,
          });
        ...
        if (responseObj.records && Array.isArray(responseObj.records)) {
          this.logger?.debug('Normalized records property', {
            itemCount: responseObj.records.length,
          });
      ...
      if (opts.logWarnings) {
        this.logger?.warn(
          'Unable to normalize response to array, unknown format',
          {
      ...
      this.logger?.error('Error during array normalization', {
        error,
        response,
      });
  ...
  extractSingleItem<T>(
    ...
    try {
      ...
      if (response === null || response === undefined) {
        this.logger?.debug(
          'Received null/undefined response for single item extraction'
        );
        return null;
      }
      ...
      if (this.isApiResponse(response)) {
        ...
        return this.extractSingleItem<T>(response.data, {
          ...opts,
          logWarnings: false,
        });
      }
      ...
      if (this.isPaginatedResponse(response)) {
        ...
        if (items.length === 0) {
          this.logger?.debug(
            'Paginated response data is empty, returning null'
          );
          return null;
        }
      ...
      if (response && typeof response === 'object') {
        ...
        for (const prop of arrayProperties) {
          if (responseObj[prop] && Array.isArray(responseObj[prop])) {
            ...
            if (items.length > 0) {
              this.logger?.debug(`Extracted first item from ${prop} property`, {
                totalItems: items.length,
              });
              return items[0] ?? null;
            } else {
              this.logger?.debug(
                `${prop} property is empty array, returning null`
              );
              return null;
            }
          }
      ...
      if (opts.logWarnings) {
        this.logger?.warn(
          'Unable to extract single item, unknown response format',
          {
            responseType: typeof response,
      ...
      this.logger?.error('Error during single item extraction', {
        error,
        response,
      });
  ...
  normalizeCount(response: unknown, options?: NormalizationOptions): number {
    ...
    try {
      ...
      if (response === null || response === undefined) {
        this.logger?.debug(
          'Received null/undefined response for count, returning 0'
        );
        return 0;
      }
      ...
      if (typeof response === 'number') {
        if (response < 0) {
          if (opts.logWarnings) {
            this.logger?.warn('Received negative count, using 0 instead', {
              count: response,
            });
          }
          return 0;
        }
        this.logger?.debug('Using direct number response as count', {
          count: response,
        });
      ...
      if (this.isPaginatedResponse(response)) {
        ...
        if (typeof total === 'number') {
          this.logger?.debug('Extracted count from paginated response meta', {
            total,
          });
      ...
      if (
        ...
        for (const field of countFields) {
          ...
          if (typeof value === 'number') {
            ...
            this.logger?.debug(`Extracted count from ${field} field`, {
              count,
            });
            return count;
          }
        ...
        for (const field of arrayFields) {
          if (Array.isArray(responseObj[field])) {
            ...
            this.logger?.debug(`Using ${field} array length as count`, {
              count,
            });
            return count;
          }
      ...
      if (typeof response === 'string') {
        ...
        if (!isNaN(parsed) && isFinite(parsed)) {
          ...
          if (opts.logWarnings) {
            this.logger?.warn('Parsed string as count', {
              original: response,
              parsed: count,
            });
          }
      ...
      if (opts.logWarnings) {
        this.logger?.warn(
          'Unable to extract count from response, returning 0',
          {
            responseType: typeof response,
            isArray: Array.isArray(response),
            keys:
      ...
      this.logger?.error('Error during count normalization', {
        error,
        response,
      });
  ...
  normalizeResponse<T>(
    ...
    try {
      this.logger?.debug('Normalizing response for operation', {
        operation,
        responseType: typeof response,
      });
      ...
      this.logger?.error('Error during response normalization', {
        error,
        operation,
        response,
      });
  ...
  extractMetadata(
    ...
    try {
      ...
      this.logger?.error('Error during metadata extraction', {
        error,
        response,
      });
   ...
   */
  handleNotFound<T>(
    operation: OperationType = 'findOne'
  ): T | T[] | number | null {
  ...
  handleMalformedResponse<T>(
    ...
    if (strict) {
      throw new Error(
        `ResponseNormalizer: Unable to handle malformed response for ${operation}: ${error.message}`
      );
    }
  ...
  validateResponseForOperation(
    ...
    try {
      ...
      switch (operation) {
        case 'findOne':
          ...
          return (
            ...
            (Array.isArray(response) &&
              (response.length <= 1 || opts.allowEmpty)) ||
      ...
      this.logger?.error('Error during response validation', {
        error,
        response,
        operation,
      });
    ...
    try {
      ...
      if (typeof response !== 'object') {
        if (opts.logWarnings && this.logger) {
          this.logger.warn('Cannot extract count from non-object response', {
            response,
            type: typeof response,
          });
        }
      ...
      if (opts.logWarnings && this.logger) {
        this.logger.warn(
          'Could not extract count from response, defaulting to 0',
          { response }
        );
      }
      ...
      if (opts.logWarnings && this.logger) {
        this.logger.warn('Error normalizing count response', {
          error,
          response,
        });
      }
      ...
      if (opts.strict) {
        throw new Error(
          `Failed to normalize count response: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
